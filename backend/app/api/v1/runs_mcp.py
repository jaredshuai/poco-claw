...
@router.get("/{run_id}/lifecycle-events")
def list_lifecycle_events(
    ...
    events = (
        db.query(AgentRunLifecycleEvent)
        .filter(AgentRunLifecycleEvent.run_id == run_id)
        .order_by(AgentRunLifecycleEvent.created_at.asc())
        .all()
    )
    return Response.success(
        data=[

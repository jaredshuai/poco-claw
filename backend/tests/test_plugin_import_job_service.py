...
from app.schemas.plugin_import import PluginImportCommitResponse
...
class TestPluginImportJobServiceProcessCommitJob(unittest.TestCase):
    """Test PluginImportJobService.process_commit_job."""
    ...
    def _make_job(self) -> MagicMock:
        ...
        job.status = "queued"
        job.progress = 0
        job.archive_key = "imports/plugins/archive.zip"
        job.selections = [{"relative_path": "plugins/example/plugin.json"}]
    ...
    @patch("app.services.plugin_import_job_service.PluginImportJobRepository")
    @patch("app.services.plugin_import_job_service.SessionLocal")
    def test_process_commit_job_uses_clock_for_success_timestamps(
        ...
        self.import_service.commit.return_value = PluginImportCommitResponse()
        service = PluginImportJobService(

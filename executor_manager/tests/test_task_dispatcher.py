...
class TestTaskDispatcherDispatch:
                    ...
                    with patch("app.scheduler.task_dispatcher.ConfigResolver"):
                        with patch("app.scheduler.task_dispatcher.SkillStager"):
                            with patch("app.scheduler.task_dispatcher.PluginStager"):
                                with patch("app.scheduler.task_dispatcher.AttachmentStager"):
                                    with patch("app.scheduler.task_dispatcher.SlashCommandStager"):
                                        with patch("app.scheduler.task_dispatcher.SubAgentStager"):
    ...
    async def test_dispatch_success(self) -> None:
        ...
        with patch("app.scheduler.task_dispatcher.get_settings") as mock_settings:
            ...
            mock_skill_stager.stage_skills = MagicMock(return_value={"skill1": {"staged": True}})

            mock_plugin_stager = MagicMock()
            mock_plugin_stager.stage_plugins = MagicMock(return_value={"plugin1": {"staged": True}})

            mock_attachment_stager = MagicMock()
            mock_attachment_stager.stage_inputs = MagicMock(return_value=[{"staged": True}])
            ...
            with patch("app.scheduler.task_dispatcher.ExecutorClient", return_value=mock_executor_client):
                with patch("app.scheduler.task_dispatcher.BackendClient", return_value=mock_backend_client):
                    with patch("app.scheduler.task_dispatcher.ConfigResolver", return_value=mock_config_resolver):
                        with patch("app.scheduler.task_dispatcher.SkillStager", return_value=mock_skill_stager):
                            with patch("app.scheduler.task_dispatcher.PluginStager", return_value=mock_plugin_stager):
                                with patch("app.scheduler.task_dispatcher.AttachmentStager", return_value=mock_attachment_stager):
                                    with patch("app.scheduler.task_dispatcher.SlashCommandStager", return_value=mock_slash_command_stager):
                                        with patch("app.scheduler.task_dispatcher.SubAgentStager", return_value=mock_subagent_stager):
                                            with patch.object(
                                                TaskDispatcher,
                                                ...
                                                mock_backend_client.update_session_status.assert_called_once_with("session-456", "running")
    ...
    async def test_dispatch_with_exception(self) -> None:
        ...
        with patch("app.scheduler.task_dispatcher.get_settings") as mock_settings:
            ...
            mock_executor_client = MagicMock()
            mock_executor_client.execute_task = AsyncMock(side_effect=RuntimeError("Executor failed"))
                                        ...
                                        with patch("app.scheduler.task_dispatcher.SubAgentStager", return_value=mock_subagent_stager):
                                            with patch.object(
                                                ...
                                                with patch.object(
                                                    ...
                                                    mock_backend_client.update_session_status.assert_called_with("session-456", "failed")
                                                    mock_pool.cancel_task.assert_called_once_with("session-456")
                                    ...
                                    with patch("app.scheduler.task_dispatcher.SlashCommandStager", return_value=mock_slash_command_stager):
                                        with patch("app.scheduler.task_dispatcher.SubAgentStager", return_value=mock_subagent_stager):
                                            with patch.object(
                                                ...
                                                assert call_kwargs["sdk_session_id"] == "sdk-session-789"
                                    ...
                                    with patch("app.scheduler.task_dispatcher.SlashCommandStager", return_value=mock_slash_command_stager):
                                        with patch("app.scheduler.task_dispatcher.SubAgentStager", return_value=mock_subagent_stager):
                                            with patch.object(
                                                ...
                                                import time
                                                enqueued_at = time.perf_counter() - 0.1  # 100ms ago
    ...
    async def test_dispatch_subagent_stager_exception(self) -> None:
        ...
        with patch("app.scheduler.task_dispatcher.get_settings") as mock_settings:
            ...
            mock_subagent_stager = MagicMock()
            mock_subagent_stager.stage_raw_agents = MagicMock(side_effect=ValueError("Invalid agent"))
